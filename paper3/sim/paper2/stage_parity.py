...
def energy_index(p, k):
    """Stage parasitic energy per unit sheave mass, in units of m_s * v_payload^2."""
    return p / float(k * k)
...
def main():
    ...
    for src, val in [("parent FIG. 8", 3), ("parent FIG. 9", 7),
        ...
        p = (val - 1) / 2.0
        note = "p = %.0f" % p if val % 2 else "EVEN, p = %.0f, the degenerate small case" % (val / 2.)
