"""Near-balanced k=7 designs with a larger width budget (2.7 m and 3.3 m per array), N=7 and N=9. Writes balanced_study3.json."""
...
K = 7
for N, W in ((7, 2.7), (9, 2.7), (7, 3.3), (9, 3.3)):
    b = None
    for sd in range(10):
    ...
    key = f"N{N}_W{W}"; out[key] = {"single": dict(rms=float(rms1), pR=e1["pR"], pC=e1["pC"], vC=e1["vC"], width=float(2*Rst.sum()))}
    print(f"N={N} W={W}: single one-line rms {rms1:.3f} pR {e1['pR']:.2f} pC {e1['pC']:.3f} vC {e1['vC']:.1f} width {2*Rst.sum():.2f}")
    for lam in (0.03, 1.0, 3.0, 10.0):
        ...
        out[key][str(lam)] = rec
        print(f"   lam={lam:<5} rms {rec['rms']:.3f}  lateral {100*ll:5.2f}% ({rec['lateral_kN']:4.1f} kN)  term {rec['pct']:.1f}%  pR {e['pR']:.2f}  pC {e['pC']:.3f}  vC {e['vC']:.1f}  widths {rec['width_lo']:.2f}/{rec['width_hi']:.2f}  il {ok}")
json.dump(out, open("balanced_study3.json", "w"), indent=1)
